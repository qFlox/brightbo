...
const prefix = {
  ...
  run: async (client, message, args) => {
    const userID = args[0];
    if (!userID) {
      return message.reply("Please specify a user to view their warnings.");
    }
...
const slash = {
  data: new SlashCommandBuilder()
    .setName("warnings")
    .setDescription("View a user's warnings in this server.")
    .addUserOption((option) => option.setName("user").setDescription("The user to view their warnings.").setRequired(true)),
  cooldown: 5000,
  ownerOnly: false,
